...
"""
#Set 2
params['epsilong']=25e5
params['epsilons']=15e5
params['pig0']=20e5
params['pis0']=15e5
params['bg']=1.4e-8
params['bs']=-2.1e-8
...
"""
#Set 4
params['epsilong']=23e5
params['epsilons']=10e5
params['pig0']=20e5
params['pis0']=20e5
params['bg']=1.6e-8
params['bs']=-2.0e-8
...
def delwiche_dynamic(P_sol, t, params):
    ...
    cta=params['cta']
    Rp=params['Rp']
    pig=params['pig']*(1-np.exp(-7.8*0.01*t))
    Rr=params['Rr'] #Added
    Ct=params['Ct'] #Added
    ...
    *A3**2*k*Rp/(Vs0*A5)*(1+pis0/epsilons)\
    ...
    #dpsitdt=(1/(Ct*Rp)*(-Pm+pim0*(1-Pm/epsilonm)+psit)+1/(Ct*Rr)*(psir-psit))
    dpsitdt=(1/(Ct*Rp)*(+Pm-pim0*(1-Pm/epsilonm)-psit)+1/(Ct*Rr)*(psir-psit))
    
    #dpsitdt=(1/(Ct*Rp)*(-Pm*(1+pim0/epsilonm)+pim0+psit)+1/(Ct*Rr)*(psir-psit))
    #dpsitdt=(1/(Ct*Rp)*(-Pm-pim0+psit)+1/(Ct*Rr)*(psir-psit))
    #dpsitdt=(1/(Ct*Rp)*(Pm-pim0*(1-Pm/epsilonm)-psit)+1/(Ct*Rr)*(psir-psit)) # From corentin's code
    
    return([dPgdt,dPsdt,dPmdt,dpsitdt])


P_sol=[100000,100000,100000,100000]
...
trace11 = go.Scatter(x=time/60, y=pig_t/100000, marker={'color': 'black', 'symbol': 104, 'size': 10}, 
                    mode="lines", name='Guard cell Osmotic pressure (bar)')
...
trace41 = go.Scatter(x=time/60, y=psit/100000, marker={'color': 'blue', 'symbol': 104, 'size': 10}, 
                    mode="lines", name='Psi_t')
trace42 = go.Scatter(x=time/60, y=psi_g/100000, marker={'color': 'black', 'symbol': 104, 'size': 10}, 
                    mode="lines", name='Psi_g')
trace43 = go.Scatter(x=time/60, y=psi_s/100000, marker={'color': 'green', 'symbol': 104, 'size': 10}, 
                    mode="lines", name='Psi_s')
data=go.Data([trace41,trace42,trace43])
figure=go.Figure(data=data)
figure.update_layout(
    xaxis_title="Time (min)",
